...
int main() {
  ...
  // Start in lane 1
  int lane = 1;
  double reference_velocity = 1.5; //mph
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			/* TODO [rana@] Remove this with actual values:
			double pos_x;
			double pos_y;
			double angle;
			...
			*/
			...
			for (int i = 0; i < sensor_fusion.size(); i++)
			{
				...
				if (d < (2 + 4 * lane + 2) && d >(2 + 4 * lane - 2))
				{
					...
					{
						// TODO we need to do some logic here to reduce vel. smoothly
			...
			// List of widely spaced way points
			vector<double> ptsx;
			vector<double> ptsy;
			...
			// Create a fucking spline yay!
			tk::spline sp_line;

			// set x and y points to the spline
			sp_line.set_points(ptsx, ptsy);

			// Starting from the last points of my previous iteration
			...
			double target_y = sp_line(target_x);
